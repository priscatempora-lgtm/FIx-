...
class EditorViewModel(
     ...
     * started. Never written to by live-slider rendering Ã¢â‚¬â€ only used as the
     ...
     * renders Ã¢â‚¬â€ both proceed, both recycle the same old bitmap, and the app
    ...
    // Latest confirmed slider values Ã¢â‚¬â€ used by [bakeFullResBeauty] so the
   ...
   // Vault to protect the pre-background image from being overwritten
     ...
     * Crop Ã¢â€ â€™ Retouch Ã¢â€ â€™ Background cycle and is about to return to LayoutConfig.
     * [PhotoSizeAdjustmentScreen] collects this event to append the new image Ã¢â‚¬â€
     * never automatically on [finalCroppedBitmap] change Ã¢â‚¬â€ preventing ghost
    ...
    private companion object {
         ...
         * Keeps the C++ engine working on Ã¢â€°Â¤ 2 MP instead of 12 MP during
         * interactive editing.
         */
    ...
    /**
     * Single-image Text "Done": bake the overlays into the saved image so the
     * completed image in History and the next output step both use the edited
     * pixels rather than the pre-text source.
     */
        suspend fun bakeTextForSingleImage(context: Context): String? =
        withContext(Dispatchers.IO) {
            try {
                val layers = _textLayers.value
                val currentPath = _projectState.value?.savedImagePath ?: return@withContext null
                
                if (layers.isEmpty()) {
                    return@withContext currentPath
                }

                // THE FIX: Use the live memory image, don't read from disk.
                val source = _finalCroppedBitmap.value?.takeUnless { it.isRecycled } ?: return@withContext currentPath

                val baked = withContext(Dispatchers.Default) {
                    bakeTextLayersOntoBitmap(source, layers, context)
                }

                val file = File(context.filesDir, "${projectId}_text_${java.util.UUID.randomUUID()}.png")
                file.outputStream().use { out ->
                    check(baked.compress(Bitmap.CompressFormat.PNG, 100, out)) { "Could not write text bitmap" }
                }
                val bakedPath = file.absolutePath

                val current = _projectState.value ?: return@withContext bakedPath
                val updated = current.copy(
                ...
                // WE DELETED setCroppedBitmap() AND clearTextLayers() HERE!
                // The screen now stays fully editable when you navigate back.
    ...
    // =========================================================================
    // BEAUTY ENGINE Ã¢â‚¬â€ SESSION START & LIVE PREVIEW
    // =========================================================================
     ...
     *
     * On the first call:
     * 1. Captures [originalBeautySource] Ã¢â‚¬â€ the full-resolution pristine copy.
     * 2. Creates [previewBitmap] Ã¢â‚¬â€ a downscaled copy (max [PREVIEW_MAX_DIM] px)
     ...
     * silently ignored Ã¢â‚¬â€ the render will begin as soon as [cachedFaceData] is
     * populated.
     */
     ...
     *
     * [_finalCroppedBitmap] is never touched during live preview Ã¢â‚¬â€ it retains
    ...
    private fun renderBeautyPreview(
        ...
        beautyRenderJob = viewModelScope.launch(Dispatchers.Default) {
            ...
            // lock multiple coroutines can run the native call concurrently Ã¢â‚¬â€ causing concurrent
            ...
            renderMutex.withLock {
                ...
                if (outputPixels.size == totalPixels) {
                    ...
                    // DEFERRED recycle Ã¢â‚¬â€ do NOT call oldPreview.recycle() here.
                    //
                    ...
                    // buffer while the GPU is reading it Ã¢â€ â€™ "Canvas: trying to use a
    ...
    // =========================================================================
    // FULL-RES BAKE Ã¢â‚¬â€ called from RetouchScreen on "Next"
    // =========================================================================
     ...
     * 3. Executes the C++ beauty pass Ã¢â€ â€™ produces the full-res baked bitmap.
    ...
    suspend fun bakeFullResBeauty(context: Context): Bitmap? {
        ...
        return withContext(Dispatchers.Default) {
            try {
                // 2. Full-resolution face scan Ã¢â‚¬â€ masks must match source dimensions.
                val analyzer      = FaceAnalysisRepository(context)
    ...
    // =========================================================================
    // BEAUTY SESSION Ã¢â‚¬â€ COMMIT / CANCEL / CLEAR
    // =========================================================================
     ...
     * Equivalent to [clearBeautySession] Ã¢â‚¬â€ the caller must already have set
     ...
     *  2. Restore the pristine crop to the UI immediately (safe Ã¢â‚¬â€ reads
    ...
    fun cancelBeautySession() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            //    Only inside this lock is it safe to recycle Ã¢â‚¬â€ no native or ML
    ...
    // =========================================================================
    // BACK NAVIGATION Ã¢â‚¬â€ Background Ã¢â€ â€™ Retouch
    // =========================================================================
     ...
     */
     fun restoreRetouchCheckpoint(context: Context) {
        // ðŸ”¥ 1. SYNCHRONOUS WIPE BEFORE SUSPENDING
    ...
    fun updateCropRatio(newRatio: CropRatio) {
        // In-memory only Ã¢â‚¬â€ no DB write here. The project is persisted by
    ...
    fun updateRotation(degrees: Float) {
        // In-memory only Ã¢â‚¬â€ same deferred-save policy as updateCropRatio.
        _projectState.update { current ->
     ...
     *
     * Called:
     *  - by [OutputSelectionScreen] after a successful digital-copy save   Ã¢â€ â€™ DIGITAL
     *  - by [PhotoSizeAdjustmentScreen] on its auto-save LaunchedEffect     Ã¢â€ â€™ PRINT
     */
    ...
    fun setBatchMode(paperSizeId: String) {
        ...
        if (currentBatchGroupId == null) {
            ...
            // double-card bug â€” but once a 2nd photo joins, the original
     ...
     * back to [PhotoSizeAdjustmentScreen] Ã¢â‚¬â€ never from inside the screen itself.
     */
     ...
     */
       fun clearCurrentSession() {
        ...
        // CHANGED: _finalCroppedBitmap.value is whatever RetouchScreen is
        // ACTIVELY PAINTING whenever no beauty preview exists yet (i.e. before
        // the user has ever tapped "Face Retouch"). Recycling it synchronously
        // in the same call that nulls the StateFlow races Compose/Skia's
        // still-in-flight GPU frame for that exact bitmap — the same hazard
        // already documented and avoided for beautyPreview below, and for
        // restoreRetouchCheckpoint elsewhere in this file. State is nulled
        // here immediately (safe); the actual recycle is deferred.
        val orphanedFinal = _finalCroppedBitmap.value

        val snapshotPreview = previewBitmap
        previewBitmap = null
        val snapshotSource = originalBeautySource
        originalBeautySource = null
        cachedFaceData = null
        ...
        viewModelScope.launch(Dispatchers.Main) {
            delay(100L)
            orphanedFinal?.let { if (!it.isRecycled) it.recycle() }
            if (oldPreview != null && oldPreview !== snapshotPreview && !oldPreview.isRecycled) {
                oldPreview.recycle()
            }
            snapshotPreview?.let { if (!it.isRecycled) it.recycle() }
            snapshotSource?.let { if (!it.isRecycled) it.recycle() }
     ...
     * real content when launched as standalone edits from "My Studio" Ã¢â‚¬â€ i.e.
    ...
    suspend fun loadImageForStandaloneEdit(
        ...
        withContext(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                // pixels Ã¢â‚¬â€ reset the in-memory rotation to 0Ã‚Â° so the crop slider starts
     ...
     * appropriate callback Ã¢â‚¬â€ the caller's navigation code always runs on Main.
     */
    fun saveDigitalCopy(
        context: android.content.Context,
        bitmap: Bitmap,
        ...
        viewModelScope.launch {
            val success = withContext(Dispatchers.IO) {
                try {
                    ...
                    // Every export is now its own History entry â€” the underlying
                    ...
                    // its own card â€” clear it from the original project so re-opening
    ...
    withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (!_isBatchMode.value || isFirstBatchImage) {
                ...
                // This branch was missing entirely â€” meaning photo 2+ in any batch
                ...
                if (existing != null) {
                    // Second+ save for the SAME photo within one session (e.g. re-baking) â€”
                    // update in place rather than minting a duplicate.
                    val updated = existing.copy(savedImagePath = path, projectType = ProjectType.PRINT)
                    repository.saveProject(updated)
                    // FIX (batch escape hatch): _projectState must reflect this photo's
                    // own savedImagePath, or clearCurrentSession()'s cleanup filter
                    // (which keys off _projectState.value?.savedImagePath) silently
                    // no-ops and leaves this image stuck in printBatches forever.
                    _projectState.value = updated
                } else {
                    val newCard = ProjectState(
                        originalImagePath = path,
                    ...
                    repository.saveProject(newCard)
                    // Same fix as above â€” the live StateFlow must track this
                    // batch photo's own card, not stay stuck on whatever
                    // resetForNewBatchImage() last set (savedImagePath = null).
                    _projectState.value = newCard
                }
            }
     ...
     * Publishes this project to History as PRINT â€” the deferred counterpart
     ...
     * tool session can pick it up, without publishing it to History â€” that
    ...
    suspend fun saveStagingBitmapToPrivateFile(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                // standalone crop's ratio stick permanently â€” discard only ever
    ...
    fun resumeBatchSession(draft: PrintProjectDraft) {
        // Idempotent â€” re-navigating back to Layout Config after adding another
    ...
    /** Called on successful export â€” batch is resolved, its draft (if any) is stale. */
