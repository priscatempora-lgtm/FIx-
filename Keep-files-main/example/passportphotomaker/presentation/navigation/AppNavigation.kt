    ...
    NavHost(navController = navController, startDestination = Routes.HOME) {
        ...
        navigation(
            ...
            composable(
                ...
                // FIX (batch escape-hatch flash): the multi-hop popBackStack used by
                // onEscapeBatchMode crossfades this screen out against whatever the
                // destination (Layout Config) is already showing, which is what reads
                // as a "flash" of the previous photo mid-transition. No animation on
                // the pop makes it an instant, correct cut instead.
                ...
                CropAlignScreen(
                    ...
                    onEscapeBatchMode  = {
                        editorViewModel.clearCurrentSession()
                        navController.popBackStack(
                            route = "${Routes.CROP_ALIGN}/{projectId}?isStandaloneEdit={isStandaloneEdit}",
                            inclusive = true
                        )
          ...
          // SCREEN D ── Textscreen ──────────────────────────────────────────
            composable(
                ...
                TextEditorScreen(
                    ...
                    onNavigateToNext = { targetDpi ->  // <-- DPI Parameter added here!
                        coroutineScope.launch {
                            if (isStandaloneEdit) {
                                // MUST KEEP: Takes you back to My Studio
                                navController.popBackStack(
                                    "${Routes.PROJECT_DETAILS}/$projectId",
                                    inclusive = false
                                )
                            } else if (batchPaperId != null) {
                                val bakedPath = editorViewModel.bakeTextForBatch(context)
                                if (bakedPath != null) {
                                    editorViewModel.commitBatchImage(bakedPath)
                                }
                                navController.popBackStack(
                                    route = "${Routes.CROP_ALIGN}/{projectId}?isStandaloneEdit={isStandaloneEdit}",
                                    inclusive = true
                                ...
                                // 1. Scale the background image to high-res
                                editorViewModel.scaleBitmapToDpi(targetDpi)
                                
                                // 2. Bake the text on top of it
                                val bakedPath = editorViewModel.bakeTextForSingleImage(context)
                                (bakedPath ?: editorViewModel.projectState.value?.savedImagePath)?.let {
                                    editorViewModel.commitBatchImage(it)
                                }
                                
                                // 3. Move forward to output
                                navController.navigate("${Routes.OUTPUT_SELECTION}/$projectId?targetDpi=$targetDpi")
                            }
                        }
                    },
                    isStandaloneEdit = isStandaloneEdit,
            ...
            composable(
                ...
                OutputSelectionScreen(
                    ...
                    targetDpi             = targetDpi,
                    onNavigateBack        = {
                        // THE FIX: Reverted to standard pop so it goes back to Text Editor
                        navController.popBackStack()
                    },
                    onPrintLayoutSelected = { paperId, resolvedUri ->
                        if (!resolvedUri.isNullOrEmpty()) {
                            ...
                            navController.navigate(
                                // Removes 'isStandaloneEdit=true' so Layout Config knows to go Home on back press
                                "${Routes.PHOTO_SIZE_ADJUSTMENT}/$projectId/$paperId?seedImageUri=$encoded"
                            ) {
                                popUpTo("${Routes.OUTPUT_SELECTION}/$projectId") { inclusive = true }
                            }
                        } else {
                            navController.navigate(
